...
let buildGmailFilterQuery, buildSetupEmailHtml, buildGmailFilterPrefillUrl;

beforeAll(() => {
  ({ buildGmailFilterQuery, buildSetupEmailHtml, buildGmailFilterPrefillUrl } = loadAppsScript(
    ["Onboarding.js"],
    ["buildGmailFilterQuery", "buildSetupEmailHtml", "buildGmailFilterPrefillUrl"],
    {
...
describe("buildSetupEmailHtml", () => {
  it("includes the filter query inside a <pre> block", () => {
    var html = buildSetupEmailHtml("from:(a@b.com)", "bot@gmail.com", "https://demo", "https://guide", null);
    expect(html).toMatch(/<pre[^>]*>[\s\S]*from:\(a@b\.com\)[\s\S]*<\/pre>/);
  });

  it("includes the bot inbox address", () => {
    var html = buildSetupEmailHtml("q", "bot@gmail.com", "https://demo", "https://guide", null);
    expect(html).toContain("bot@gmail.com");
  });

  it("includes both the demo and guide links", () => {
    var html = buildSetupEmailHtml("q", "bot@gmail.com", "https://demo.example", "https://guide.example", null);
  ...
  it("escapes HTML-significant characters in the query", () => {
    var html = buildSetupEmailHtml('a<b>&"c', "bot@gmail.com", "https://demo", "https://guide", null);
